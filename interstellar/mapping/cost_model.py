...
def get_block_size(point, layer, level):
    ...
    for i in range(le.NUM):
        ...
        partitioning_accum_list.append(
            reduce(mul, point.loop_partitioning(i)[:level + 1], 1))  # FIXME inclusive mode also duplicates data
        ...
        for i in range(len(cap)):
            ...
            if invalid_underutilized:
                ...
                if None not in last_layer:
                    if ((size <= cap[i]) and (2 * size <= cap[
                        i])) == True:  # if double the size fit then there will be a better to block partition that will utilized all memory,
...
def valid_mapping_point_current_level(resource, point, layer, level, verbose=False):
    if resource.paras[level].count > 1:
        ...
        valid_size = fit_in_level(resource.buffer(level).capacity,
                                  get_block_size(point, layer, level), resource.invalid_underutilized, level,
                                  resource.memory_partitions)
...
def valid_blocking_size_current_level(resource, point, layer, level, verbose=False):
    ...
    if type(resource.buffer(level).capacity) is list:
        ...
        return fit_in_level(capacity, get_block_size(point, layer, level),
                            (resource.invalid_underutilized and (level not in resource.para_index)), level,
                            resource.memory_partitions)
    else:
